...
## Recent Changes
- **Nov 24, 2025 (Security Update)**: Enhanced tracking endpoints with automatic customer_id filtering. Removed customer_id query parameter - now automatically extracted from JWT token for customer users. This prevents customers from accessing vehicles belonging to other customers. Applied to all tracking endpoints: /tracking/vehicles, /tracking/vehicles/{id}/location, /tracking/vehicles/{id}/history, /tracking/vehicles/{id}/route.
